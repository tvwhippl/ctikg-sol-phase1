...
logger = logging.getLogger("topic_candidate_select")
...
def select_topics(
    ...
    # Supports plain .txt plus JSON/JSONL corpora (e.g., results/scraped_corpus.jsonl).
    texts: List[str] = []
    seed_indexes: List[int] = []

    def _add_seed_text(t: str) -> None:
        nonlocal texts, seed_indexes
        if not t:
            return
        texts.append(t)
    ...
    if seed_article_paths_or_ids:
        for pth in seed_article_paths_or_ids:
            pth = str(pth)
            try:
                if os.path.exists(pth):
                    ext = Path(pth).suffix.lower()
                    if ext == ".jsonl":
                        # Each line is a JSON object; extract a likely text field.
                        for line in Path(pth).read_text(encoding="utf-8", errors="replace").splitlines():
                            ...
                            try:
                                ...
                                # allow raw-text jsonl
                                _add_seed_text(line)
                                continue
                            if isinstance(obj, dict):
                                for k in ("text", "content", "body", "article_text", "raw_text", "snippet", "title"):
                                    val = obj.get(k)
                                    if isinstance(val, str) and val.strip():
                                        _add_seed_text(val.strip())
                                        break
                            elif isinstance(obj, str):
                                _add_seed_text(obj.strip())
                    elif ext == ".json":
                        obj = json.loads(Path(pth).read_text(encoding="utf-8", errors="replace"))
                        if isinstance(obj, list):
                            for item in obj:
                                if isinstance(item, str):
                                    _add_seed_text(item.strip())
                        ...
                        _add_seed_text(Path(pth).read_text(encoding="utf-8", errors="replace"))
                else:
                    # treat as doc-id or literal snippet placeholder
                    _add_seed_text(pth)
            except Exception as e:
                logger.warning("Couldn't read %s: %s", pth, e)

    # Always include the user query as a short "document" to help deterministic methods.
    # IMPORTANT: if we prepend the query, shift seed indexes accordingly.
    if user_query_or_seed_keywords:
        texts = [user_query_or_seed_keywords] + texts
        seed_indexes = [i + 1 for i in seed_indexes]

    if not texts:
        texts = [user_query_or_seed_keywords] if user_query_or_seed_keywords else []
